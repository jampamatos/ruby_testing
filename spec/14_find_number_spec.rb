...
describe FindNumber do
  ...
  describe '#make_guess' do
    ...
    context 'when min is 0 and max is 9' do
      it 'returns 4' do
        min = 0
        max = 9
        guess = game_guessing.make_guess(min, max)
  ...
  describe '#game_over?' do
    let(:answer_number) { double('answer_number') }
    subject(:game_is_over) { described_class.new(0, 9, answer_number, 8) }
    context 'when guess and random_number are equal' do
      ...
      it 'is game over' do
        allow(answer_number).to receive(:value).and_return(8)
    ...
    context 'when guess and random_number are not equal' do
      it 'is not game over' do
        allow(answer_number).to receive(:value).and_return(9)
        expect(game_is_over).not_to be_game_over
      end
    end
  end
  ...
  describe '#update_range' do
    ...
    context 'when the guess is 7, min=5, and max=8' do
      ...
      it 'updates min to the same value as max' do
        ...
        expect(edge_guess_game.min).to eq(edge_guess_game.max)
      end

      it 'does not update max' do
        expect(edge_guess_game.max).to eq(8)
      end
    end
  end
end
